...
location_filter):
  ...
  2. Si el texto de ubicación + department contiene alguna exclude_keyword
     (ej. "us only", "onsite", "india", "europe") -> rechaza, sin importar remote_flag.
...
Se evalúa location + department juntos porque varias empresas globales
(ATS tipo Greenhouse/Lever de multinacionales) codifican la región real
en el nombre del equipo/departamento (ej. "APAC Engineering", "India
Talent") en vez de en location.
"""


def is_location_compatible(location: str, remote_flag, cfg: dict, department: str = "") -> bool:
    ...
    combined_lc = f"{location or ''} {department or ''}".lower().strip()

    exclude_kw = cfg.get("exclude_keywords", [])
    if any(kw.lower() in combined_lc for kw in exclude_kw):
        return False

    allow_kw = cfg.get("allow_keywords", [])
    if any(kw.lower() in combined_lc for kw in allow_kw):
        return True
